...
function satelliteMaxDim(satKey, sectorKey) {
  ...
  if (phone || touchTablet || lowMemory) return 1200;
  // A Himawari full disk is 5500×5500 and GeoColor needs four decoded bands.
  // Capping the working grid avoids a several-hundred-megabyte burst that can
  // terminate otherwise capable desktop tabs while retaining useful map detail.
  if (satKey === 'himawari9' && sectorKey === 'hfd') return 1800;
  return 0;
}
...
export function loadSceneAsync(satKey, sectorKey, key, bands, onProgress, bbox = null) {
  return call({ type: 'load', satKey, sectorKey, key, bands, bbox, maxDim: satelliteMaxDim(satKey, sectorKey) }, onProgress)
